const App = {
    ...
    saveState() {
        ...
        localStorage.setItem('crisis_craft_data', JSON.stringify(CrisisData));
    },
    ...
    loadState() {
        ...
        const savedData = localStorage.getItem('crisis_craft_data');
        if (savedState) {
        ...
        if (savedData) {
            ...
            Object.assign(CrisisData, parsedData);
        }
    },
    ...
    getLogo(size = 'medium', centered = false) {
        ...
        return `
            <div class="logo-container ${logoClass}" style="display: flex; align-items: center; justify-content: ${centered ? 'center' : 'flex-start'}; gap: 15px; margin-bottom: 30px;">
    ...
    submitLogin() {
        ...
        const roleVal = this.state.tempRole || 'admin';
        const errorEl = document.getElementById('login-error');

        const user = CrisisData.users.find(u =>
            u.user_id.toLowerCase() === userVal.toLowerCase() &&
            u.password === passVal &&
            u.role.toLowerCase() === roleVal.toLowerCase()
        );

        if (user) {
            this.navigate(roleVal);
        } else {
            if (errorEl) {
                errorEl.style.display = 'block';
                errorEl.innerHTML = `<i class="fas fa-exclamation-circle"></i> Invalid ${roleVal} credentials. Try again.`;
    ...
    renderLogin() {
        const role = this.state.tempRole || 'admin';
        return `
            <div class="login-container">
                ...
                <div class="input-group">
                    ...
                    <input type="text" id="login-user" class="input-style" placeholder="Enter your ID">
                </div>
                <div class="input-group">
                ...
                <div style="margin-bottom: 30px;">
                    <p style="color: var(--text-secondary); font-size: 0.85rem; margin-bottom: 12px; text-align: left;">Select Role</p>
                    <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 10px;">
                        <button onclick="App.state.tempRole = 'admin'; App.render()" class="btn" style="background: ${role === 'admin' ? 'var(--cyan)' : 'var(--bg-navy)'}; border: ${role === 'admin' ? 'none' : '1px solid var(--navy-light)'}; color: ${role === 'admin' ? 'var(--bg-navy)' : 'white'}; font-weight: ${role === 'admin' ? 'bold' : 'normal'};">Admin</button>
                        <button onclick="App.state.tempRole = 'teacher'; App.render()" class="btn" style="background: ${role === 'teacher' ? 'var(--indigo)' : 'var(--bg-navy)'}; border: ${role === 'teacher' ? 'none' : '1px solid var(--navy-light)'}; color: white; font-weight: ${role === 'teacher' ? 'bold' : 'normal'};">Teacher</button>
                        <button onclick="App.state.tempRole = 'student'; App.render()" class="btn" style="background: ${role === 'student' ? 'var(--purple)' : 'var(--bg-navy)'}; border: ${role === 'student' ? 'none' : '1px solid var(--navy-light)'}; color: white; font-weight: ${role === 'student' ? 'bold' : 'normal'};">Student</button>
                    </div>
                </div>
                <button onclick="App.submitLogin()" class="btn btn-primary">Login Now</button>
                </div>
            </div>
        `;
    },
    ...
    renderHeader() {
        ...
        const user = CrisisData.users.find(u => u.role.toLowerCase() === role.toLowerCase());
        const fullName = user ? `${user.u_name.fname} ${user.u_name.lname}` : (this.state.profiles[role]?.name || 'User');
            ...
            admin: `
                ...
                <div class="glass" style="padding:30px; margin-bottom:20px;">
                    ...
                    <div style="display:flex; gap:15px; flex-wrap:wrap;">
                        <button class="btn" style="background:rgba(239,68,68,0.1); color:var(--red); border:1px solid rgba(239,68,68,0.3); width:auto; padding:10px 20px;" onclick="if(confirm('Reset all student quiz scores? This cannot be undone.')) { CrisisData.quizzes.forEach(q => { q.studentScore = null; q.status = 'Open'; }); App.showToast('All quiz scores reset successfully.', 'success'); App.saveState(); App.render(); }">Reset All Quiz Scores</button>
                        <button class="btn" style="background:rgba(239,68,68,0.1); color:var(--red); border:1px solid rgba(239,68,68,0.3); width:auto; padding:10px 20px;" onclick="if(confirm('Clear entire drill history? This cannot be undone.')) { CrisisData.drills = CrisisData.drills.filter(d => d.status === 'Registered'); App.showToast('Drill history cleared.', 'success'); App.saveState(); App.render(); }">Clear Drill History</button>
                        <button class="btn" style="background:rgba(239,68,68,0.2); color:var(--red); border:1px solid rgba(239,68,68,0.5); width:auto; padding:10px 20px; font-weight:bold;" onclick="if(confirm('⚠ FACTORY RESET: This will erase ALL platform data. Are you absolutely sure?')) { localStorage.clear(); location.reload(); }">Factory Reset Platform</button>
                    </div>
                </div>`,
            teacher: `
                <div class="glass" style="padding:30px; margin-bottom:20px;">
    ...
    exportCSV(type) {
        let csv = '', filename = '';
        if (type === 'roster') {
            csv = 'ID,Name,Role,Institution,Status\n' + CrisisData.users.map(u => `${u.user_id},${u.u_name.fname} ${u.u_name.lname},${u.role},${u.i_id},${u.status}`).join('\n');
            filename = 'student_roster.csv';
        } else if (type === 'grades') {
            csv = 'Name,Quiz Score,Status\n' + CrisisData.quizzes.map(q => `"${q.title}",${q.studentScore || 'N/A'},${q.status}`).join('\n');
            filename = 'grade_sheet.csv';
        } else if (type === 'drill') {
            csv = 'Drill Name,Participation,Avg Score,Status,Date\n' + CrisisData.drills.map(d => `"${d.title}",${d.participation}%,${d.score}%,${d.status},${d.scheduled_date}`).join('\n');
    ...
    getAdminOverview() {
        const stats = CrisisData.statistics.admin;
        return `
            <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 20px; margin-bottom: 30px;">
                ${this.renderStatCard("Total Students", stats.totalStudents, "fa-user-graduate", "var(--cyan)")}
                ${this.renderStatCard("Total Teachers", stats.totalTeachers, "fa-chalkboard-teacher", "var(--indigo)")}
                ${this.renderStatCard("Active Drills", stats.activeDrills, "fa-vr-cardboard", "var(--purple)")}
                ${this.renderStatCard("Readiness Score", stats.readinessScore + "%", "fa-shield-alt", "var(--cyan)")}
            </div>
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 25px; margin-bottom: 30px;">
                <div class="glass" style="padding: 30px; height: 350px; display: flex; flex-direction: column;">
                    <h3 style="margin-bottom: 20px;">Participation</h3>
                    <div style="flex: 1; position: relative;"><canvas id="drillChart"></canvas></div>
                </div>
                <div class="glass" style="padding: 30px; height: 350px; display: flex; flex-direction: column;">
                    <h3 style="margin-bottom: 20px;">Awareness</h3>
                    <div style="flex: 1; position: relative;"><canvas id="awarenessChart"></canvas></div>
                </div>
            </div>
            ${this.getAlertPanel()}
        `;
        ...
        return `
            <div class="glass" style="padding: 30px;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px;">
                    <h3>Manage Users</h3>
                    <button class="btn btn-primary" style="width: auto; padding: 10px 20px;" onclick="App.showAddUserModal()">+ Add User</button>
                </div>
                <table style="width: 100%; border-collapse: collapse; text-align: left;">
                    <thead>
                        <tr style="color: var(--text-secondary); font-size: 0.85rem; border-bottom: 1px solid var(--glass-border);">
                            <th style="padding: 15px 0;">NAME</th><th style="padding: 15px 0;">ROLE</th><th style="padding: 15px 0;">STATUS</th><th style="padding: 15px 0;">ACTION</th>
                        </tr>
                    </thead>
                    <tbody>
                        ${CrisisData.users.map((user, index) => {
            const fullName = `${user.u_name.fname} ${user.u_name.lname}`;
            const inst = CrisisData.institutions.find(i => i.i_id === user.i_id)?.i_name || 'Main Campus';
            return `
                            <tr style="border-bottom: 1px solid var(--glass-border);">
                                <td style="padding: 15px 0;">
                                    <div style="display:flex; align-items:center; gap:12px;">
                                        <div style="width:32px; height:32px; border-radius:50%; background:rgba(0,245,255,0.1); display:flex; align-items:center; justify-content:center; color:var(--cyan); font-weight:bold;">${user.u_name.fname[0]}</div>
                                        <div>
                                            <p style="font-weight:500;">${fullName}</p>
                                            <p style="font-size:0.7rem; color:var(--text-secondary);">${inst}</p>
                                        </div>
                                    </div>
                                </td>
                                <td style="padding: 15px 0;"><span style="background: rgba(255,255,255,0.05); padding: 5px 10px; border-radius: 8px; font-size: 0.8rem;">${user.role}</span></td>
                                <td style="padding: 15px 0;">
                                    <div style="display: flex; align-items: center; gap: 8px;">
                                        <div style="width: 6px; height: 6px; border-radius: 50%; background: ${user.status === 'Active' ? '#22c55e' : '#ef4444'};"></div>
                                        <span>${user.status}</span>
                                    </div>
                                </td>
                                <td style="padding: 15px 0;">
                                    ${user.role !== 'Admin' ? `<i class="fas fa-trash" style="color: var(--red); cursor: pointer;" onclick="App.removeUser(${index})" title="Remove User"></i>` : '<i class="fas fa-lock" style="color: grey; opacity: 0.5;" title="Admin Locked"></i>'}
                                </td>
                            </tr>`;
        }).join('')}
                    </tbody>
        ...
        return `
            <div class="glass" style="padding: 30px;">
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 20px;">
                    ${CrisisData.drills.map((drill, idx) => `
                        <div class="glass" style="padding: 20px; border: 1px solid rgba(0, 245, 255, 0.2); position: relative;">
                            <i class="fas fa-trash" style="position: absolute; top: 20px; right: 20px; color: var(--red); cursor: pointer; opacity: 0.7; transition: 0.2s;" onmouseover="this.style.opacity=1" onmouseout="this.style.opacity=0.7" onclick="App.removeDrill(${idx})"></i>
                            <h4 style="margin-bottom: 10px; padding-right: 20px;">${drill.title}</h4>
                            <div style="display: flex; flex-direction: column; gap: 8px; font-size: 0.9rem; color: var(--text-secondary); margin-bottom: 15px;">
                                <span><i class="fas fa-calendar-alt" style="width: 20px;"></i> ${drill.scheduled_date}</span>
                                <span><i class="fas fa-exclamation-circle" style="width: 20px;"></i> Type: ${drill.disaster_type}</span>
                                <span><i class="fas fa-users" style="width: 20px;"></i> ${drill.participation}% Participation</span>
                            </div>
                            <span style="background: ${drill.status === 'Completed' ? 'rgba(34, 197, 94, 0.1)' : 'rgba(0, 245, 255, 0.1)'}; color: ${drill.status === 'Completed' ? '#22c55e' : 'var(--cyan)'}; padding: 5px 12px; border-radius: 12px; font-size: 0.8rem; border: 1px solid ${drill.status === 'Completed' ? 'rgba(34, 197, 94, 0.3)' : 'rgba(0, 245, 255, 0.3)'};">${drill.status}</span>
                        </div>
                    `).join('')}
                </div>
            </div>
    ...
    // --- Upload Modules ---
    getUploadModules() {
        if (!this.state.uploadedModules) this.state.uploadedModules = [...CrisisData.modules];
        ...
        return `
            <div style="display:grid; gap:25px;">
                <div class="glass" style="padding:35px;">
                    ...
                    <div id="selectedFileInfo" style="margin-top:20px; display:none;">
                        ...
                        <div style="margin-top:15px;">
                            ...
                            <button class="btn btn-primary" style="width:auto; padding:12px 30px;" onclick="App.postModule()">
                ...
                <div class="glass" style="padding:35px;">
                    <h3 style="margin-bottom:20px;">Posted Modules <span style="color:var(--text-secondary); font-size:0.85rem; font-weight:normal;">(${mods.length})</span></h3>
                    <div style="display:grid; gap:12px;">
                        ${mods.map((m, i) => `
                            <div class="glass" style="padding:18px 22px; display:flex; align-items:center; justify-content:space-between; border:1px solid ${m.completion === 100 ? 'rgba(34,197,94,0.2)' : 'rgba(0,245,255,0.15)'}; transition:0.2s;" onmouseover="this.style.transform='translateX(4px)'" onmouseout="this.style.transform='translateX(0)'">
                                <div style="display:flex; align-items:center; gap:15px; flex:1;">
                                    ...
                                    <div style="flex:1;">
                                        ...
                                        <div style="display:flex; gap:15px; font-size:0.8rem; color:var(--text-secondary);">
                                            <span><i class="fas fa-users" style="margin-right:4px;"></i>${m.students} students</span>
                                            <span><i class="fas fa-chart-line" style="margin-right:4px;"></i>${m.completion}% complete</span>
                                            <span style="color:${m.status === 'Completed' ? '#22c55e' : m.status === 'Not Started' ? 'var(--text-secondary)' : 'var(--cyan)'};">${m.status}</span>
                                        </div>
                                    </div>
                                </div>
                                <div style="display:flex; gap:10px; align-items:center;">
                                    <div style="width:80px; height:4px; background:rgba(255,255,255,0.05); border-radius:2px; overflow:hidden;">
                                        <div style="width:${m.completion}%; height:100%; background:${m.completion === 100 ? '#22c55e' : 'var(--cyan)'};"></div>
                                    </div>
                                    <i class="fas fa-trash" style="color:var(--red); cursor:pointer; opacity:0.6; transition:0.2s; font-size:0.85rem;" onmouseover="this.style.opacity=1" onmouseout="this.style.opacity=0.6" onclick="App.removeModule(${i})"></i>
                                </div>
                            </div>
                        `).join('')}
                    </div>
    ...
    postModule() {
        ...
        if (!this.state.uploadedModules) this.state.uploadedModules = [...CrisisData.modules];
        this.state.uploadedModules.unshift({ title, students: 0, completion: 0, status: 'Not Started', studentProgress: 0 });
        CrisisData.statistics.teacher.modulesUploaded = this.state.uploadedModules.length;
    ...
    removeModule(index) {
        ...
        if (!this.state.uploadedModules) this.state.uploadedModules = [...CrisisData.modules];
        this.state.uploadedModules.splice(index, 1);
        CrisisData.statistics.teacher.modulesUploaded = this.state.uploadedModules.length;
    ...
    getTeacherOverview() {
        ...
        return `
            <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 20px; margin-bottom: 30px;">
                ...
                ${this.renderStatCard("Modules Uploaded", stats.modulesUploaded, "fa-book", "var(--indigo)")}
            ...
            <div style="display: grid; grid-template-columns: 1.5fr 1fr; gap: 25px;">
                <div class="glass" style="padding: 30px;">
                    ...
                    ${CrisisData.modules.map(mod => `
                        <div style="margin-bottom:15px;">
                            <div style="display:flex; justify-content:space-between; margin-bottom:5px;"><span>${mod.title}</span><span style="color:var(--cyan);">${mod.completion}%</span></div>
                            <div style="width:100%; height:6px; background:rgba(255,255,255,0.05); border-radius:3px; overflow:hidden;"><div style="width:${mod.completion}%; height:100%; background:var(--cyan);"></div></div>
                        </div>
                    `).join('')}
                </div>
        ...
        if (section === 'Learning Modules') {
            return `
                <div class="glass" style="padding: 40px;">
                    ...
                    <div style="display:grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap:20px;">
                        ${CrisisData.modules.map((m, i) => `
                            <div class="glass glass-card" style="padding:25px; border:1px solid ${m.studentProgress === 100 ? 'rgba(34,197,94,0.2)' : 'rgba(0,245,255,0.15)'};">
                                <div style="display:flex; justify-content:space-between; align-items:start; margin-bottom:15px;">
                                    <div style="width:45px; height:45px; border-radius:12px; background:rgba(0,245,255,0.1); display:flex; align-items:center; justify-content:center;">
                                        <i class="fas ${m.studentProgress === 100 ? 'fa-check-circle' : 'fa-book-open'}" style="color:var(--cyan); font-size:1.2rem;"></i>
                                    </div>
                                    <span style="font-size:0.8rem; font-weight:600; color:${m.studentProgress === 100 ? '#22c55e' : 'var(--cyan)'};">${m.studentProgress}%</span>
                                </div>
                                ...
                                <div style="width:100%; height:6px; background:rgba(255,255,255,0.05); border-radius:3px; overflow:hidden; margin-bottom:20px;">
                                    <div style="width:${m.studentProgress}%; height:100%; background:linear-gradient(90deg, var(--cyan), var(--indigo));"></div>
                                </div>
                                <button class="btn ${m.studentProgress === 100 ? '' : 'btn-primary'}" style="width:100%; padding:10px; font-size:0.85rem; background:${m.studentProgress === 100 ? 'rgba(255,255,255,0.05)' : ''};" onclick="App.startModule(${i})">
                                    ${m.studentProgress === 100 ? 'Review Module' : m.studentProgress > 0 ? 'Continue' : 'Start Now'}
                                </button>
                            </div>
                        `).join('')}
                    </div>
    ...
    sendAlert() {
        ...
        if (input && input.value.trim() !== '') {
            if (!this.state.activeAlerts) this.state.activeAlerts = [];
            this.state.activeAlerts.push(input.value.trim());
            input.value = '';
    ...
    dismissAlert(msg) {
        if (this.state.activeAlerts) {
            this.state.activeAlerts = this.state.activeAlerts.filter(a => a !== msg);
            this.render();
        }
    },
